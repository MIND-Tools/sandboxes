...
public class CompilerRunner {
	...
	public CompilerRunner(final Map<Object, Object> initialContext)
		...
		// SSZ: Addition to the original tests setup: We need to tell the tests to use the "--optimize" (-O) option
		// to enable this module for tests, else we can't test the module...
		// Inspired from the standard Launcher.
	...
	public File annotateDefinitionCompile(final String adlName, final String executableName, final Collection<Annotation> annotations)
			throws ADLException, InterruptedException {
		String outputPath;
		if (executableName != null) {
			context.put("executable-name", executableName);
		...
		if (annotations != null) {
			for (Annotation anno : annotations) {
				AnnotationHelper.addAnnotation(d, anno);
			}
		}
	...
	public void addCFlags(List<String> flags) {
		...
		CompilerContextHelper.addCFlags(context, flags);
	}
