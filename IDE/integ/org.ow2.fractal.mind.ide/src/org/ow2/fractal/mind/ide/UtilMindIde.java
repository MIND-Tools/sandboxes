...
public class UtilMindIde {
	...
	public static MindRepo findOrCreateRepo(MindModel mm, String name) {
		...
		if (wsMindRoot == null) {
			...
			wsMindRoot.setMindId(mindRepository.eClass().getEPackage().getNsPrefix()+"/"+name);
	...
	static public MindPackage findOrCreatePackage(MindRootSrc rs, String pn, IResource resrcPackage, boolean create) {
	...
	public static MindPackage createPackage(MindRootSrc rs, String pn, IResource resrcPackage) {
		MindPackage packageFound;
	...
	public static MindRootSrc findRootSrc(MindRepo mindRepo, IPath fullPath) {
		return UtilMindIde.find(mindRepo.getRootsrcs(), fullPath.toPortableString());
	}

	public static MindAdl findAdl(EList<MindFile> files, String nameFile) {
		return (MindAdl) findFile(files, nameFile, MindidePackage.Literals.MIND_ADL);
	}
	...
	public static String[] appliInfo(String name ) throws CoreException {
		...
		if (colonIndex == -1) {
			throw new CoreException(new Status(Status.ERROR, MindActivator.ID, "incomplete entry application is missing "+name));
		}
