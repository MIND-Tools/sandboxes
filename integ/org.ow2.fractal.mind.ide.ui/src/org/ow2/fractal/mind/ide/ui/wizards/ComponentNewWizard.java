...
/**
 * Create a Fractal Mind ADL Component
 */
...
public class ComponentNewWizard extends Wizard implements INewWizard {
	...
	private void doFinish(
		...
		monitor.beginTask(NLS.bind(Messages.ComponentNewWizard_create_adl, packageName+"."+componentName), 2); //$NON-NLS-2$
		IWorkspaceRoot root = ResourcesPlugin.getWorkspace().getRoot();
		IResource resource = root.findMember(new Path(containerName));
		if (!resource.exists() || !(resource instanceof IContainer)) {
			throwCoreException(NLS.bind(Messages.error_container_does_not_exists , containerName )); //$NON-NLS-2$
		}
