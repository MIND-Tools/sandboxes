...
public class ModelToProjectUtil {
	...
	public static URI resolveAdl(String componentName, EList<String> imports) {
		MindProject project = getMindProject();
		String defaultPackage = getPackage(project).toString();
		MindAdl adl = project.resolveAdl(componentName, defaultPackage, imports);
		URI resourceURI = URI.createURI(adl.getFullpath());
		return resourceURI;
	}
	
	public static List<String> getDefinitionsInProject() {
	...
	private static void refreshEditorInput () {
		IWorkbenchWindow window = PlatformUI.getWorkbench().getActiveWorkbenchWindow();
		IWorkbenchPage page = window.getActivePage();
		editorInput = page.getActiveEditor().getEditorInput();
	}
	
	public static MindProject getMindProject() {
		return getMindProject(getIProject());
	}
	
	public static MindProject getMindProject(IProject project) {
		return MindIdeCore.get(project);
	}
	
	public static String getCurrentDefinition() {
		// TODO		
		return new String(" ");
	}
	 ...
	 * @return the IProject associated with the editorInput
	 */
	public static MindPackage getPackage(MindProject project) {
		refreshEditorInput();
			...
			try{
				...
				if (relativeURI.segmentCount() > 1) {
					...
					for (MindRootSrc rootSrc : roots) {
						...
						if (rootPath == convertedRelativePath.substring(0, rootPath.length())) {
							...
							for (MindPackage mindPackage : packages) {
								...
								if (packagePath == convertedRelativePath.substring(0, packagePath.length())) {
									return mindPackage;
								}
							}
						}
